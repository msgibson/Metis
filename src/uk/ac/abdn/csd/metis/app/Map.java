...
public class Map {
     ...
     * @param nodeFactory 
     */
    public Map(Model model, String prefix, int width, int height, NodeFactory nodeFactory) {
        // TODO check parameters. width and height should be > 0.
    	this.model = model;
    	this.prefix = prefix;
        this.nodeFactory = nodeFactory;        
        nodes = (Tile[][]) new Tile[width][height];
        this.width = width - 1;
    ...
    private void initEmptyNodes() {
        for (int i = 0; i <= width; i++) {
            for (int j = 0; j <= height; j++) {
                nodes[i][j] = (Tile) nodeFactory.createNode(model, prefix, i, j);
            }
        }
    }
    ...
    public AppResource getResource(int x, int y){
    	return nodes[x][y].getHasResource();
    }
    
    public void setResource(int x, int y, AppResource raw){
    	nodes[x][y].setHasResource(raw);
    }
    
    public boolean getVisited(int x, int y){
    	return nodes[x][y].getBeenVisited();
    }
    
    public void setVisited(int x, int y, boolean bool){
    	nodes[x][y].setBeenVisited(bool);
    }
     ...
     * @return node
     */
    public final Tile getNode(int x, int y) {        
        return nodes[x][y];
    }    
     ...
     * @return
     */
    public final List<Tile> findPath(int oldX, int oldY, int newX, int newY) {
        // TODO check input
        ...
        while (!done) {
            ...
            List<Tile> adjacentNodes = getAdjacent(current);
            for (int i = 0; i < adjacentNodes.size(); i++) {
                Tile currentAdj = adjacentNodes.get(i);
            ...
            if (openList.isEmpty()) { // no path exists
                return new LinkedList<Tile>(); // return empty list
            }
        }
     ...
     */
    private List<Tile> getAdjacent(Tile node) {
        // TODO make loop
        int x = node.getXPosition();
        int y = node.getYPosition();
        List<Tile> adj = new LinkedList<Tile>();
