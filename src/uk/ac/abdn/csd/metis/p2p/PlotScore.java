...
public class PlotScore implements Control{
	...
	public PlotScore(String prefix){
	...
	public boolean execute() {	
		...
		for(int i = 0; i < Network.size(); i++){			
			...
			try{
				PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(i + "score.csv", true)));
				if(CommonState.getPhase() == CommonState.POST_SIMULATION){
					out.println(CommonState.getTime() + "," + (player.getTotalScore()));
				}else{
					out.println(CommonState.getTime()-1 + "," + (player.getTotalScore()));
				}
				out.close();
			}catch(Exception e){
				e.printStackTrace();
